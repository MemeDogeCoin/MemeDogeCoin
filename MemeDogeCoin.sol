...
contract MemeDoge is ERC20, Ownable {
    ...
    event UpdateWallets(address oldMarketAddress, address newMarketAddress);
    event ExcludeFromFees(address indexed account, bool isExcluded);
    event ExcludeMultipleAccountsFromFees(address[] accounts, bool isExcluded);
    event ExcludeFromMaxTxLimit(address indexed account, bool isExcluded);
    event ExcludeMultipleAccountsFromMaxTxLimit(address[] accounts, bool isExcluded);
    event SetSwapTokensAtAmount(uint256 OldAmount, uint256 NewAmount);

    constructor() ERC20("Meme Doge Coin", "MemeDoge") {
        ...
        excludeFromAllLimits(burnAddress, true);

        /*
            _mint is an internal function in ERC20.sol that is only called here,
            and CANNOT be called ever again
        */
        _mint(owner(), 100 * 10**9 * (10**decimals()));
        maxTransactionAmount = totalSupply().div(100);
    }

    function updatePancakeRouter(address router) private {
    ...
    function setMarketAddress(address newMarketAddress) public onlyOwner {
        require(newMarketAddress != address(0), 'Marketing Zero address');
        emit UpdateWallets(marketAddress, newMarketAddress);
        marketAddress = newMarketAddress;
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "MemeDoge: Account is already the value of 'excluded'");
    ...
    function excludeFromMaxTxLimit(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromMaxTxLimit[account] != excluded, "MemeDoge: Account is already the value of 'excluded'");
        _isExcludedFromMaxTxLimit[account] = excluded;

        emit ExcludeFromMaxTxLimit(account, excluded);
    }

    function excludeMultipleAccountsFromMaxTxLimit(address[] calldata accounts, bool excluded) public onlyOwner {
        for(uint256 i = 0; i < accounts.length; i++) {
            _isExcludedFromMaxTxLimit[accounts[i]] = excluded;
        }

        emit ExcludeMultipleAccountsFromMaxTxLimit(accounts, excluded);
    }
    ...
    function _transfer(
        ...
        if(takeFee) {
            uint256 forMarket = amount.mul(marketFee).div(100);
            super._transfer(from, address(this), forMarket);

            uint256 forBurn = amount.mul(burnFee).div(100);
            super._transfer(from, burnAddress, forBurn);

            amount = amount.sub(forMarket).sub(forBurn);
        }
